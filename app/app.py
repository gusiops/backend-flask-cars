...
class Order(db.Model):
    __tablename__ = 'order'  # Имя таблицы
    id = db.Column(db.Integer, primary_key=True)  # Первичный ключ
    name = db.Column(db.String)  # Имя клиента
    car = db.Column(db.String)  # Название автомобиля
    phone = db.Column(db.String)  # Номер телефона
...
def create_order_car():
    ...
    if not customer_name or not car or not phone:
        return jsonify({"error": "Missing required fields"}), 400

    # Создаем новый заказ
    new_order = Order(name=customer_name, car=car, phone=phone)
